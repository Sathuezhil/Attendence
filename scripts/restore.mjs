...
import { createFirestoreFromEnv, restoreFirestore } from './lib/firestore.mjs';

loadEnvFiles();

if (process.env.CONFIRM_RESTORE !== 'YES') {
  console.error(
    'Refusing to restore. Set CONFIRM_RESTORE=YES and pass BACKUP_FILE, GOOGLE_DRIVE_BACKUP_FILE_ID, or GOOGLE_DRIVE_BACKUP_KEY.',
  );
  console.error('This script is for operators only. Do not expose it over HTTP.');
  process.exit(1);
}
...
try {
  ...
  console.log('Sign in on the mobile app and check one employee, document, invoice, and payroll record.');
  console.log('Employee files stay in Google Drive; they are not rewritten by this restore.');
} catch (error) {
